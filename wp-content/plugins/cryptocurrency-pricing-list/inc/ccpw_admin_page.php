...
function ccpw_list_all_currencies(){
	...
	// Coin List
	$ccpw_coinlist_url = 'https://min-api.cryptocompare.com/data/all/coinlist';
	$ccpw_coinlist_response = wp_remote_get($ccpw_coinlist_url,array('headers' => array( 'Content-Type' => 'application/json' ),'timeout' => 200));
	if ( is_wp_error( $ccpw_coinlist_response ) ) {
	$ccpw_coinlist_response = wp_remote_get($ccpw_coinlist_url,array('headers' => array( 'Content-Type' => 'application/json' ),'timeout' => 200));
	}else{
	$ccpw_coinlist_responsed = wp_remote_retrieve_body($ccpw_coinlist_response); 
	}
	$ccpw_coinlist_all_currencies_raw = json_decode($ccpw_coinlist_responsed, true);
	...
	function myFilter($var){return ($var !== '');}
	...
	$ccpw_crypto_algo = array_values(array_filter($_POST['ccpw_data-algo'],'myFilter'));
	$ccpw_crypto_proof = array_values(array_filter($_POST['ccpw_data-proof'],'myFilter'));
	$ccpw_crypto_supply = array_values(array_filter($_POST['ccpw_data-supply'],'myFilter'));
	$ccpw_crypto_fullname = array_values(array_filter($_POST['ccpw_data-fullname'],'myFilter'));
	...
	if(!empty($_GET['ccpw_setpage']) && $_GET['ccpw_setpage']==2){$ccpw_setpage = ($_GET['ccpw_setpage']-1)+100;$maxlimit=$_GET['ccpw_setpage']*100;}
	elseif(!empty($_GET['ccpw_setpage'])&& $_GET['ccpw_setpage']>=3){
		$ccpw_setpage = ($_GET['ccpw_setpage']-1)*100+1;$maxlimit=$_GET['ccpw_setpage']*100;
	} else{$ccpw_setpage = 0;$maxlimit=100;}
...
$ccpw_display_currencies = get_post_meta( $post_id, 'ccpw_display_currencies' );
...
foreach ($ccpw_display_currencies as $ccpw_display_currencies_array) {
	...
	<label class="ccpw_img_none">
	<input type="checkbox" name="ccpw_display_currencies[]" value="'.$value.'" '.((is_array( $ccpw_display_currencies_array ) && in_array($value, $ccpw_display_currencies_array))? 'checked':'').'> 
	'.$value.'
	</label>
	</p>';
}
...
function cd_meta_box_save( $post_id )
{
   ...
   update_post_meta( $post_id, 'ccpw_display_currencies', $ccpw_save_crypto_currencies);
