...
$data_url = 'https://min-api.cryptocompare.com/data/histoday?fsym='.$unserialize_cry_currency.'&tsym='.$unserialize_currency.'&limit='.$ccpw_total_day.'&aggregate=1&e=CCCAGG';
...
echo '<ul>';
$data_url_1 = 'https://min-api.cryptocompare.com/data/pricemultifull?fsyms='.$unserialize_cry_currency.'&tsyms='.$unserialize_currency.'';
$data_url_1response = wp_remote_get($data_url_1,array('headers' => array( 'Content-Type' => 'application/json' ),'timeout' => 200));
if ( is_wp_error( $data_url_1response ) ) {
$data_url_1response = wp_remote_get($data_url_1,array('headers' => array( 'Content-Type' => 'application/json' ),'timeout' => 200));
}else{		
$data_url_1responsed = wp_remote_retrieve_body($data_url_1response);
}
$data_all_currencies_raws_1 = json_decode($data_url_1responsed, true);
