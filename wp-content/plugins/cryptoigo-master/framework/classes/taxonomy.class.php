 ...
 *
 */
class CRYPTOIGOFramework_Taxonomy extends CRYPTOIGOFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'CRYPTOIGO_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && CRYPTOIGO_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = CRYPTOIGO_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $CRYPTOIGO_errors = get_transient( 'cryptoigo-taxonomy-transient' );

    wp_nonce_field( 'cryptoigo-taxonomy', 'cryptoigo-taxonomy-nonce' );

    echo '<div class="cryptoigo-framework cryptoigo-taxonomy cryptoigo-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo CRYPTOIGO_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( CRYPTOIGO_get_var( 'cryptoigo-taxonomy-nonce' ), 'cryptoigo-taxonomy' ) ) {

      $errors = array();
      $taxonomy = CRYPTOIGO_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'CRYPTOIGO_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'CRYPTOIGO_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'CRYPTOIGO_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'CRYPTOIGO_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'CRYPTOIGO_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'cryptoigo-taxonomy-transient', $errors, 10 );
