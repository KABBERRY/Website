 ...
 *
 */
class CRYPTOIGOFramework_Shortcode_Manager extends CRYPTOIGOFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'CRYPTOIGO_shortcode_options', $options );
    $this->exclude_post_types = apply_filters( 'CRYPTOIGO_shortcode_exclude', $this->exclude_post_types );
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_cryptoigo-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && CRYPTOIGO_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary cryptoigo-shortcode" data-editor-id="'. $editor_id .'">'. esc_html__( 'Add Shortcode', 'cryptoigo-framework' ) .'</a>';
    }
  ...
  ?>
    <div id="cryptoigo-shortcode-dialog" class="cryptoigo-dialog" title="<?php esc_html_e( 'Add Shortcode', 'cryptoigo-framework' ); ?>">
      <div class="cryptoigo-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>cryptoigo-dialog-select" data-placeholder="<?php esc_html_e( 'Select a shortcode', 'cryptoigo-framework' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="cryptoigo-dialog-load"></div>
      <div class="cryptoigo-insert-button hidden">
        <a href="#" class="button button-primary cryptoigo-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'cryptoigo-framework' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    $request = CRYPTOIGO_get_var( 'shortcode' );
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        echo CRYPTOIGO_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="cryptoigo-shortcode-clone" data-clone-id="'. $clone_id .'">';
      echo '<a href="#" class="cryptoigo-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      echo '<div class="cryptoigo-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
